...
weighted_lr_low <- lm(Y~.-psvalue-weight.ATE, data = low)
...
```{r}
# We choose to delete the following 3 variables
...
weighted_lr_high <- lm(Y~.-psvalue-weight.ATE, data = high, weights = high$weight.ATE)
summary(weighted_lr_high)

# result shows we should include:
select <- names(high)%in% c('Y','A','V10', 'V28', 'V45', 'V91', 'V100','V102', 'V133', 'V138', 'V148', 'V150', 'V180')
high_delete <-high[select]
# calculate the mean of each column
...
ATE_high_selected <- lm(Y~., data = high_delete_diff , weights = high$weight.ATE)
