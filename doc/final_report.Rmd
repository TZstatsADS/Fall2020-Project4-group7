...
* Logistic Regression + Full Matching
* L1 Logistic Reg + Full Matching
* L2 Logistic Reg + Full Matching
* CART + Full Matching
* Boosting Stumps + Full Matching
...
Full matching is a type of subclassification that forms the subclasses in an optimal way. It creates a series of matched sets where each matched set contains at least one treated individual and at least one control individual. Full matching is optimal in terms of minimizing a weighted average of the estimated distance measure between each treated subject and each control subject within each subclass. Here, we use propensity score to measure the distance.
```{r, echo = FALSE, results = TRUE, message=FALSE}
...
The following table shows estimated ATEs, since we know the tree ATE for high dimensional dataset is -3 and for low dimensional dataset is 2.5. We could see that $Logistic_FullMatching$ performs best, especially on the the balanced dataset.
```{r,echo = FALSE, results = TRUE, message=FALSE, warning = FALSE}
#ATE results
ATE1=data.frame(Logistic_FullMatching = c(a,b,B),L1_FullMatching = c(c,d,D),L2_FullMatching = c(e,f,FF),CART_FullMatching = c(g,h,H),BS_FullMatching = c(ii,j,J),Logistic_WeightedReg = c(-4.13500,2.78831,NA), True_ATE = c(-3,2.5,NA),row.names = c("highdim","lowdim","balanced lowdim"))
ATE1
```
